...
export const AuthService = {
  ...
  async loginWithOtp(email: string) {
    ...
    if (error) {
      if (error.message.includes('Signups not allowed') || (error as { code?: string }).code === 'otp_disabled') {
        throw new AppError('NOT_FOUND', APP_STRINGS.AUTH.EMAIL_NOT_REGISTERED);
      }
