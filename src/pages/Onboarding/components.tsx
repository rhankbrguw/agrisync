    ...
    <form onSubmit={form.handleSubmit(onSubmit, () => toast.error(APP_STRINGS.DASHBOARD.FORM_VALIDATION_ERROR, { id: TOAST_IDS.FORM_ERROR }))} className="space-y-3 sm:space-y-4">
      <div>
        ...
        <input 
          id="workspaceCode" 
          type="text" 
          {...form.register('workspaceCode', { 
            onChange: (e) => { 
              e.target.value = e.target.value.toUpperCase().replace(/^#\s*/, '').replace(/[^A-Z0-9_-]/g, ''); 
            } 
          })} 
          placeholder={APP_STRINGS.ONBOARDING.WORKSPACE_CODE_PLACEHOLDER} 
          className="w-full h-10 sm:h-11 bg-background/50 border border-border focus:border-primary rounded-xl px-3.5 sm:px-4 text-xs sm:text-sm font-mono tracking-wider outline-none transition-all uppercase" 
        />
