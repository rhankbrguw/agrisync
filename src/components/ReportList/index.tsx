...
const ReportListHeader = ({ searchQuery, setSearchQuery, filterStatus, setFilterStatus, filterDate, setFilterDate }: HeaderProps) => (
  <div className="px-3 py-2 sm:px-5 sm:py-3 bg-surface/80 border-b border-border backdrop-blur-md flex flex-col sm:flex-row gap-2 sm:gap-3 justify-between sm:items-center z-10 relative w-full max-w-full min-w-0">
    ...
    <div className="flex items-center justify-between sm:justify-end gap-1.5 w-full sm:w-auto min-w-0">
...
export function ReportList() {
  ...
  if (isLoading) return <ReportListLoading />;
  ...
  return (
    <div className="w-full max-w-full min-w-0 h-64 sm:h-[28rem] bg-background/50 border border-border rounded-xl sm:rounded-2xl overflow-hidden flex flex-col relative shadow-sm">
      <ReportListHeader searchQuery={searchQuery} setSearchQuery={setSearchQuery} filterStatus={filterStatus} setFilterStatus={setFilterStatus} filterDate={filterDate} setFilterDate={setFilterDate} />
      <div className="flex-1 overflow-y-auto p-2.5 sm:p-5 flex flex-col bg-background/30 custom-scrollbar">
        <div className="space-y-2.5 sm:space-y-3 flex-1">
          <AnimatePresence mode="popLayout">
            {paginatedReports.length === 0 ? <ReportListEmptyState /> : (
